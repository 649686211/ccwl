...
class UserController extends HomeController {
	...
	public function index(){
        ...
        $enrollModel = D('enroll');
        $enRs=$enrollModel->field('recruit_id')->where('user_id ='.$user_id)->select();
        $ids='';
        if($enRs){
            foreach($enRs as $key=>$val){
                $ids.=$val['recruit_id'].",";
            }
        }
        $ids=rtrim($ids,',');
	...
	public function register($username = '', $password = '', $repassword = '', $email = '', $verify = ''){
		...
		if(IS_POST){ //注册用户
			...
			if(0 < $uid){ //注册成功
				...
				$this->error($this->showRegError($uid).'错误：'.$uid);
			}
	...
	public function login($username = '', $password = '', $verify = ''){
		if(IS_POST){ //登录验证
			...
			if(0 < $uid){ //UC登录成功
				//写入cookie //30天过期
				$cookie_key=md5('hubing_uid');
				$cookie_val=md5('hubing_'.mt_rand()).$uid;
				cookie($cookie_key,$cookie_val,3600*24*30);
				/* 登录用户 */
				$Member = D('Member');
